...
<template match="/">
    <ns0:schematron-output xmlns:ns0="http://purl.oclc.org/dsdl/svrl" schemaVersion="" title="">
      ...
      <ns0:ns-prefix-in-attribute-values prefix="m" uri="http://www.ociweb.com/movies" />
      <ns0:active-pattern>
...
<!--PATTERN -->
	...
	<!--RULE -->
<template match="m:actor" mode="M1" priority="1000">
    <ns0:fired-rule xmlns:ns0="http://purl.oclc.org/dsdl/svrl" context="m:actor" />

		<!--REPORT -->
<if test="@role=preceding-sibling::m:actor/@role">
      <ns0:successful-report xmlns:ns0="http://purl.oclc.org/dsdl/svrl" test="@role=preceding-sibling::m:actor/@role">
        <attribute name="location">
          <apply-templates mode="schematron-select-full-path" select="." />
        </attribute>
        <ns0:text>
        Duplicate role!
      </ns0:text> <ns0:diagnostic-reference diagnostic="duplicateActorRole">

      More than one actor plays the role<text />
          <value-of select="@role" />
          <text />.
      A duplicate is named<text />
          <value-of select="@name" />
          <text />.
    </ns0:diagnostic-reference>
      </ns0:successful-report>
    </if>
