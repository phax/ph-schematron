...
<transform xmlns="http://www.w3.org/1999/XSL/Transform" version="2.0">
  <ns0:Description xmlns:ns0="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
    ...
    <ns1:created xmlns:ns1="http://purl.org/dc/terms/">2023-02-10T17:48:54.3539251+01:00</ns1:created>
  </ns0:Description>
  <output indent="yes" />
  <template match="root()">
      ...
      <ns0:metadata xmlns:ns0="http://purl.oclc.org/dsdl/svrl">
        ...
        <ns1:source xmlns:ns1="http://purl.org/dc/terms/">
          <ns2:Description xmlns:ns2="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
            ...
            <ns1:created>2023-02-10T17:48:54.3539251+01:00</ns1:created>
          </ns2:Description>
        </ns1:source>
      </ns0:metadata>
    </variable>
  ...
  <template name="w47aab7">
    <ns0:document xmlns:ns0="https://doi.org/10.5281/zenodo.1495494">
      <ns0:pattern id="w47aab7">
        <if test="exists(base-uri(root()))">
          <attribute name="documents" select="base-uri(root())" />
        </if>
        <for-each select="root()">
          <ns1:active-pattern xmlns:ns1="http://purl.oclc.org/dsdl/svrl" name="A very simple pattern with a title">
            <attribute name="documents" select="base-uri(.)" />
          </ns1:active-pattern>
        </for-each>
      </ns0:pattern>
      <apply-templates mode="w47aab7" select="root()" />
    </ns0:document>
  </template>
  <template match="chapter" mode="w47aab7" priority="0">
    <param as="xs:string*" name="schxslt:patterns-matched" />
    <choose>
      <when test="$schxslt:patterns-matched[. = 'w47aab7']">
        <ns0:rule xmlns:ns0="https://doi.org/10.5281/zenodo.1495494" pattern="w47aab7">
      ...
      <value-of select="count(para)" /> paragraphs</ns1:text>
        ...
        <next-match>
          <with-param as="xs:string*" name="schxslt:patterns-matched" select="($schxslt:patterns-matched, 'w47aab7')" />
        </next-match>
      </otherwise>
    </choose>
  </template>
