...
<transform xmlns="http://www.w3.org/1999/XSL/Transform" version="2.0">
  <ns0:Description xmlns:ns0="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
    ...
    <ns1:created xmlns:ns1="http://purl.org/dc/terms/">2023-02-10T17:48:54.6076329+01:00</ns1:created>
  </ns0:Description>
  <output indent="yes" />
  <template match="root()">
      ...
      <ns0:metadata xmlns:ns0="http://purl.oclc.org/dsdl/svrl">
        ...
        <ns1:source xmlns:ns1="http://purl.org/dc/terms/">
          <ns2:Description xmlns:ns2="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
            ...
            <ns1:created>2023-02-10T17:48:54.6076329+01:00</ns1:created>
          </ns2:Description>
        </ns1:source>
      </ns0:metadata>
    </variable>
  ...
  <template name="w102aab7">
    <ns0:document xmlns:ns0="https://doi.org/10.5281/zenodo.1495494">
      <ns0:pattern id="w102aab7">
        <if test="exists(base-uri(root()))">
          <attribute name="documents" select="base-uri(root())" />
        </if>
        <for-each select="root()">
          <ns1:active-pattern xmlns:ns1="http://purl.oclc.org/dsdl/svrl" id="doc.checks" name="doc.checks">
            <attribute name="documents" select="base-uri(.)" />
          </ns1:active-pattern>
        </for-each>
      </ns0:pattern>
      <apply-templates mode="w102aab7" select="root()" />
    </ns0:document>
  </template>
  <template match="para[@idref]" mode="w102aab7" priority="0">
    <param as="xs:string*" name="schxslt:patterns-matched" />
    <choose>
      <when test="$schxslt:patterns-matched[. = 'w102aab7']">
        <ns0:rule xmlns:ns0="https://doi.org/10.5281/zenodo.1495494" pattern="w102aab7">
          <comment>WARNING: Rule for context "para[@idref]" shadowed by preceding rule</comment>
          <ns1:suppressed-rule xmlns:ns1="http://purl.oclc.org/dsdl/svrl">
            <attribute name="context">para[@idref]</attribute>
          </ns1:suppressed-rule>
        </ns0:rule>
        <next-match>
      ...
      <otherwise>
        <ns0:rule xmlns:ns0="https://doi.org/10.5281/zenodo.1495494" pattern="w102aab7">
          ...
          <if test="not(@idref = id(@idref))">
            <ns1:failed-assert xmlns:ns1="http://purl.oclc.org/dsdl/svrl" location="{schxslt:location(.)}">
              <attribute name="test">@idref = id(@idref)</attribute>
              <ns1:text> the idref value [<value-of select="@idref" />] has no matching target</ns1:text>
            </ns1:failed-assert>
          </if>
        </ns0:rule>
        <next-match>
          <with-param as="xs:string*" name="schxslt:patterns-matched" select="($schxslt:patterns-matched, 'w102aab7')" />
        </next-match>
      </otherwise>
    </choose>
  </template>
