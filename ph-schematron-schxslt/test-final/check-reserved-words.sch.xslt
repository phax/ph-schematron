...
<transform xmlns="http://www.w3.org/1999/XSL/Transform" version="2.0">
  <ns0:Description xmlns:ns0="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
    ...
    <ns1:created xmlns:ns1="http://purl.org/dc/terms/">2022-11-17T08:43:08.672+01:00</ns1:created>
  </ns0:Description>
  <output indent="yes" />
  <template match="root()">
      ...
      <ns0:metadata xmlns:ns0="http://purl.oclc.org/dsdl/svrl">
        ...
        <ns1:source xmlns:ns1="http://purl.org/dc/terms/">
          <ns2:Description xmlns:ns2="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
            ...
            <ns1:created>2022-11-17T08:43:08.672+01:00</ns1:created>
          </ns2:Description>
        </ns1:source>
      </ns0:metadata>
    </variable>
  ...
  <template name="w1172aab1">
    <ns0:document xmlns:ns0="https://doi.org/10.5281/zenodo.1495494">
      ...
      <apply-templates mode="w1172aab1" select="root()" />
    </ns0:document>
  </template>
  <template match="Document" mode="w1172aab1" priority="0">
    <param as="xs:string*" name="schxslt:patterns-matched" />
    <choose>
      <when test="$schxslt:patterns-matched[. = 'w1172aab1']">
        <ns0:rule xmlns:ns0="https://doi.org/10.5281/zenodo.1495494" pattern="w1172aab1">
      ...
      <otherwise>
        <ns0:rule xmlns:ns0="https://doi.org/10.5281/zenodo.1495494" pattern="w1172aab1">
          ...
          <if test="not(count(//node()[contains(.,'SCRIPT')]) = 0)">
            <ns1:failed-assert xmlns:ns1="http://purl.oclc.org/dsdl/svrl" location="{schxslt:location(.)}">
              <attribute name="test">count(//node()[contains(.,'SCRIPT')]) = 0</attribute>
              <ns1:text>
             The document must not contain the word SCRIPT
         </ns1:text>
            </ns1:failed-assert>
          </if>
          <if test="not(count(//node()[contains(.,'FUNCTION')]) = 0)">
            <ns1:failed-assert xmlns:ns1="http://purl.oclc.org/dsdl/svrl" location="{schxslt:location(.)}">
              <attribute name="test">count(//node()[contains(.,'FUNCTION')]) = 0</attribute>
              <ns1:text>
        ...
        <next-match>
          <with-param as="xs:string*" name="schxslt:patterns-matched" select="($schxslt:patterns-matched, 'w1172aab1')" />
        </next-match>
      </otherwise>
    </choose>
  </template>
