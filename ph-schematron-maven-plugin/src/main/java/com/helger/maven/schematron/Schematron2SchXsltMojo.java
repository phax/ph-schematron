          ...
          final Runnable r = () -> {
            try
            {
              ...
              if (aXsltDoc != null)
              {
                ...
                // This is a workaround on the XML serialization issue, because namespace prefixes
                // that are not explicitly contained as attribute or element would be silently
                // ignored otherwise
                ...
                try (final OutputStream aOS = FileHelper.getOutputStream (aXSLTFile))
                {
                  if (aOS == null)
