...
/**
 * Test class for class {@link SchematronResourcePureXPath}.
 *
 * @author Philip Helger
 */
public final class SchematronResourcePureXPathTest
{
  private static final Logger LOGGER = LoggerFactory.getLogger (SchematronResourcePureXPathTest.class);
  ...
  public void testFromByteArray ()
  {
    final String sTest = "<?xml version=\"1.0\" encoding=\"iso-8859-1\"?>\n" +
                         ...
                         "\n" +
                         "</iso:schema>";
    assertTrue (SchematronResourcePureXPath.fromByteArray (sTest.getBytes (StandardCharsets.UTF_8))
                                           .isValidSchematron ());
    assertTrue (SchematronResourcePureXPath.fromInputStream ("ba-from-string",
  ...
  public void testParseWithXPathError ()
  {
    ...
    final SchematronResourcePureXPath aSch = SchematronResourcePureXPath.fromByteArray (sTest.getBytes (StandardCharsets.UTF_8));
    aSch.setErrorHandler (aErrorHandler);
   ...
   * default {@link EXPathVersion#XPATH_3_1} the schema binds and the assertion passes. With {@link
   * EXPathVersion#XPATH_2_0} the compiler does not know <code>fn:head</code>, so binding fails.
   */
  @Test
  public void testXPathVersionPicksFunctions () throws Exception
  {
    ...
    final SchematronOutputType aOK = SchematronResourcePureXPath.fromString (sSchema, StandardCharsets.UTF_8)
    ...
    final SchematronOutputType aOK31 = SchematronResourcePureXPath.fromString (sSchema, StandardCharsets.UTF_8)
    ...
    final CollectingPSErrorHandler aErrorHandler = new CollectingPSErrorHandler ();
    final ISchematronResource aSch20 = SchematronResourcePureXPath.fromString (sSchema, StandardCharsets.UTF_8)
  ...
  /*
   * Validates that when the XML is provided through {@link SchematronResourcePureXPath#getAsNode}
   * the document ends up parsed straight into a Saxon TinyTree exposed behind a {@link
   * net.sf.saxon.dom.NodeOverNodeInfo} facade — instead of a regular DOM document — and that
  ...
  public void testSaxonTinyTreeFastPath () throws Exception
  {
    ...
    final ISchematronResource aSCH = SchematronResourcePureXPath.fromString (sSchema, StandardCharsets.UTF_8)
    ...
    final SchematronResourcePureXPath aSCH2 = SchematronResourcePureXPath.fromString (sSchema, StandardCharsets.UTF_8);
    aSCH2.setCustomValidationHandler (aSpyHandler);
   ...
   * {@code _buildSaxonDocument} helper wraps the input through a hardened XMLReader; this test
   * feeds an XML document carrying a DOCTYPE declaration and asserts the parser refuses it up front
   * (DISALLOW_DOCTYPE_DECL fires before any external resolution would be attempted).
   */
  @Test
  public void testFastPathHardenedAgainstXXE ()
  {
    ...
    final SchematronResourcePureXPath aSCH = SchematronResourcePureXPath.fromString (sSchema, StandardCharsets.UTF_8);
  ...
  /*
   * Regression test for the security review: {@code XPathConfigBuilder.build()} must not mutate the
   * shared {@code DEFAULT_PROCESSOR} when extension functions are added without an explicit
   * processor. Otherwise a function registered for caller A leaks into every other caller using the
   * default config (cross-tenant function poisoning).
   */
  @Test
  ...
  public void testResolveVariables () throws SchematronException
  {
    ...
    assertFalse (SchematronResourcePureXPath.fromString (sTest, StandardCharsets.UTF_8)
    ...
    final SchematronOutputType aOT = SchematronResourcePureXPath.fromString (sTest, StandardCharsets.UTF_8)
  ...
  public void testResolveFunctions () throws SchematronException
  {
    ...
    final IXPathConfig aXPathConfig = new XPathConfigBuilder ().addExtensionFunction (new GetNodelistDetailsFunction ())
                                                               .build ();
    final SchematronOutputType aOT = SchematronResourcePureXPath.fromByteArray (sTest.getBytes (StandardCharsets.UTF_8))
                                                                .setXPathConfig (aXPathConfig)
                                                                .applySchematronValidationToSVRL (aTestDoc, null);
    assertNotNull (aOT);
    assertEquals (0, SVRLHelper.getAllFailedAssertions (aOT).size ());
