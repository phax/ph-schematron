...
<template match="/">
    <ns0:schematron-output xmlns:ns0="http://purl.oclc.org/dsdl/svrl" schemaVersion="" title="">
      ...
      <ns0:active-pattern>
        ...
        <attribute name="id">requiredAttribute</attribute>
        <attribute name="name">requiredAttribute</attribute>
        <apply-templates />
      </ns0:active-pattern>
...
<!--PATTERN requiredAttribute-->


	<!--RULE -->
<template match="Customer" mode="M0" priority="1000">
    <ns0:fired-rule xmlns:ns0="http://purl.oclc.org/dsdl/svrl" context="Customer" />

		<!--ASSERT -->
<choose>
      <when test="string-length(@type) > 0" />
      <otherwise>
        <ns0:failed-assert xmlns:ns0="http://purl.oclc.org/dsdl/svrl" test="string-length(@type) > 0">
        ...
        The <text />
            <value-of select="name(.)" />
            <text /> element should have a
       <text />
            <value-of select="@type/name()" />
            <text /> attribute.
      </ns0:text>
        </ns0:failed-assert>
      </otherwise>
