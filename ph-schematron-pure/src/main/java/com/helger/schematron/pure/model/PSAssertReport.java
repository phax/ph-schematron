 ...
 * An assertion made about the context nodes. The data content is a natural-language assertion. The
 * required test attribute is an assertion test evaluated in the current context. If the test
 * evaluates positive, the assertion succeeds. The optional diagnostics attribute is a reference to
 * further diagnostic information.<br>
 * The natural-language assertion shall be a positive statement of a constraint. NOTE: The
 * natural-language assertion may contain information about actual values in addition to expected
 * values and may contain diagnostic information. Users should note, however, that the diagnostic
 * element is provided for such information to encourage clear statement of the natural-language
 * assertion. <br>
                            ...
                            IPSHasLinkableGroup
{
    ...
    for (final Object aContent : m_aContent)
      if (aContent instanceof IPSElement)
        if (!((IPSElement) aContent).isValid (aErrorHandler))
          return false;
    if (StringHelper.isEmpty (m_sTest))
    {
  ...
  {
    if (StringHelper.isNotEmpty (sDiagnostics))
  ...
  /**
   * @return A list of {@link String}, {@link PSName}, {@link PSValueOf}, {@link PSEmph},
