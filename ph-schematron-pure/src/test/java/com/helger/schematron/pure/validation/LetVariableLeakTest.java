...
public final class LetVariableLeakTest
{
  ...
  /**
   * Drive two validations through the same {@link SchematronResourcePure} on the same thread. The
   * first call BREAKs immediately on the first report (leaving the schema-global {@code $totalItems}
   * still bound on the thread). The second call MUST see its own freshly-evaluated value, not the
   * first run's leftover binding.
   */
  @Test
  public void testLetBindingDoesNotLeakAfterBreak () throws Exception
  {
    final SchematronResourcePure aSCH = SchematronResourcePure.fromString (SCHEMA, StandardCharsets.UTF_8);
    ...
    final SchematronOutputType aSvrl1 = aSCH.applySchematronValidationToSVRL (com.helger.xml.serialize.read.DOMReader.readXMLDOM (_xml (5)), null);
    ...
    final SchematronResourcePure aSCH2 = SchematronResourcePure.fromString (SCHEMA, StandardCharsets.UTF_8);
    final SchematronOutputType aSvrl2 = aSCH2.applySchematronValidationToSVRL (com.helger.xml.serialize.read.DOMReader.readXMLDOM (_xml (2)), null);
