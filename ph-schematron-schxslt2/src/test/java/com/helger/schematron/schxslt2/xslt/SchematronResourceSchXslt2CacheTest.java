...
import com.helger.schematron.schxslt2.xslt.SchematronResourceSchXslt2Cache;
...
/**
 * Test class for class {@link SchematronResourceSchXslt2Cache}
 *
 * @author Philip Helger
 */
public final class SchematronResourceSchXslt2CacheTest
{
  private static final Logger LOGGER = LoggerFactory.getLogger (SchematronResourceSchXslt2CacheTest.class);
  ...
  public void testValidSynchronous () throws Exception
  {
    ...
    SchematronResourceSchXslt2.fromClassPath (VALID_SCHEMATRON);

    final long nStart = System.nanoTime ();
    for (int i = 0; i < RUNS; ++i)
    {
      final ISchematronResource aSV = SchematronResourceSchXslt2.fromClassPath (VALID_SCHEMATRON);
  ...
  {
    assertFalse (new SchematronResourceSchXslt2 (new ClassPathResource ("test-sch/invalid01.sch")).isValidSchematron ());
    assertFalse (new SchematronResourceSchXslt2 (new ClassPathResource ("test-sch/this.file.does.not.exists")).isValidSchematron ());

    assertFalse (new SchematronResourceSchXslt2 (new FileSystemResource ("src/test/resources/test-sch/invalid01.sch")).isValidSchematron ());
    assertFalse (new SchematronResourceSchXslt2 (new FileSystemResource ("src/test/resources/test-sch/this.file.does.not.exists")).isValidSchematron ());
  }
  ...
  public void testXSLTPreprocessor ()
  {
    ...
    for (final IReadableResource aRes : SchematronTestHelper.getAllValidSchematronFiles ())
    {
      ...
      final ISchematronXSLTBasedProvider aPreprocessor = SchematronResourceSchXslt2Cache.createSchematronXSLTProvider (aRes,
                                                                                                                            new TransformerCustomizerSchXslt2 ().setErrorListener (aCEH)
                                                                                                                                                                     .setLanguageCode ("de"));
